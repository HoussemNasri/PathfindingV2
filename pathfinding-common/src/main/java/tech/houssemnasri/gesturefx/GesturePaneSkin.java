...
import tech.houssemnasri.gesturefx.GesturePane.FitMode;
import tech.houssemnasri.gesturefx.GesturePane.ScrollBarPolicy;
...
import static tech.houssemnasri.gesturefx.AffineEvent.CHANGED;
import static tech.houssemnasri.gesturefx.AffineEvent.CHANGE_FINISHED;
import static tech.houssemnasri.gesturefx.AffineEvent.CHANGE_STARTED;
 ...
 *
 * <ul>
 *   <li>Content layout (vbar, hbar, target)
 *   <li>Event firing for affine change
 *   <li>Event handling for interactions
 *   <li>Clipping
 *   <li>Conditional optimisations
 * </ul>
 */
final class GesturePaneSkin extends SkinBase<GesturePane> {
    ...
    GesturePaneSkin(GesturePane control) {
        ...
        hbar.managedProperty()
                .bind(
                        isNotUnbounded.and(
                                pane.hbarPolicy
                                        .isEqualTo(ScrollBarPolicy.AS_NEEDED)
                                        .and(
                                                (hbar.visibleAmountProperty()
                                                        .greaterThan(vbar.widthProperty())))
                ...
                .bind(
                        isNotUnbounded.and(
                                pane.vbarPolicy
                                        .isEqualTo(ScrollBarPolicy.AS_NEEDED)
                                        .and(
                                                vbar.visibleAmountProperty()
                                                        .greaterThan(hbar.heightProperty()))
        ...
        vbar.minProperty()
                .bind(scaledHeight.subtract(pane.heightProperty()).add(hbarHeight).negate());
    ...
    private void setupGestures() {
        ...
        // zoom via touch
        pane.addEventHandler(ZoomEvent.ZOOM_STARTED, consumeThenFireIfEnabled(e -> markStart()));
        pane.addEventHandler(ZoomEvent.ZOOM_FINISHED, consumeThenFireIfEnabled(e -> markEnd()));
        pane.addEventHandler(
                ZoomEvent.ZOOM,
                consumeThenFireIfEnabled(
                        e -> pane.scale(e.getZoomFactor(), e.getZoomFactor(), fromGesture(e))));
    ...
    protected void layoutChildren(
        ...
        if (hbar.isManaged()) {
            layoutInArea(
                    ...
                    contentWidth
                            - (vbar.isManaged()
                                    ? (vbar.prefWidth(ScrollBar.USE_COMPUTED_SIZE))
                                    : 0),
                    contentHeight,
                    0,
                    HPos.CENTER,
        ...
        if (hbar.isManaged() && vbar.isManaged()) {
            corner.resizeRelocate(
                    hbar.getWidth(), vbar.getHeight(), hbar.getHeight(), vbar.getWidth());
        }
