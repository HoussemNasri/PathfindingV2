package tech.houssemnasri.gesturefx;
...
import tech.houssemnasri.gesturefx.AffineEvent;
import tech.houssemnasri.gesturefx.GesturePaneOps;

import static tech.houssemnasri.gesturefx.GesturePane.FitMode.FIT;
import static tech.houssemnasri.gesturefx.GesturePane.FitMode.UNBOUNDED;
import static tech.houssemnasri.gesturefx.GesturePane.ScrollBarPolicy.AS_NEEDED;
import static tech.houssemnasri.gesturefx.GesturePane.ScrollMode.PAN;

/**
 * Pane that applies transformations to some implementation of {@link Transformable} when a gesture
 * is applied
 *
 * <p>Terms:
 *
 * <ol>
 *   <li>Target - the actual object that receives transformation
 *   <li>Viewport - the view area of the target(not counting vertical and horizontal scrollbars)
 * </ol>
 *
 * <p>The measured size of the node defaults the the target's size, you can use the usual {@link
 * Region#setPrefSize(double, double)} and related methods/bindings to provide alternative
 * dimensions. To listen for transformation changes, register event listeners for {@link
 * AffineEvent}, for example:
 *
 * <pre>{@code
 * GesturePane pane = //...
 ...
 * }</pre>
 *
 * See documentation for the {@link AffineEvent} for more information on the methods provided in the
 * event
 */
...
public class GesturePane extends Control implements GesturePaneOps {
  ...
  /**
   * Creates a new {@link GesturePane} with the specified node as children(i.e the node gets added
   * to the pane)
   *
  ...
  public GesturePane() {
    ...
    final ChangeListener<Bounds> layoutBoundsListener =
        (o, p, n) -> {
  ...
  /**
   * Centre of the current viewport. <br>
   * This equivalent to :
   *
   * <pre>
   ...
   * @return a point on the target that corresponds to the viewport point or empty if the point is
   *     not within the the bound returned by {@link #getViewportBound()}
   */
  ...
  /**
   * Animate changes for all operations supported in {@link GesturePaneOps}. <br>
   * Animations does not compose so starting an new animation while one is already running will
   * result in the first animation being stopped arbitrary for the second animation to start. <br>
  ...
  /**
   * Resets zoom to the minimum scale and conditionally centres the image depending on the current
   * {@link FitMode}
   */
  public void cover() {
    fireAffineEvent(AffineEvent.CHANGE_STARTED);
    scale(
        getMinScale() / getCurrentScaleX(),
        getMinScale() / getCurrentScaleY(),
        viewportPointAt(targetPointAtViewportCentre()));
    if (fitMode.get() == FitMode.COVER) {
      ...
      if (height >= scale * getTargetHeight()) affine.setTx((width - scale * getTargetWidth()) / 2);
    }
    fireAffineEvent(AffineEvent.CHANGE_FINISHED);
  }
  ...
  private void animateValue(
    ...
    KeyValue keyValue =
        new KeyValue(
            ...
            1.0,
            interpolator == null ? Interpolator.LINEAR : interpolator);
  ...
  public double getViewportWidth() {
    return viewport.get().getWidth();
  }

  public double getViewportHeight() {
    return viewport.get().getHeight();
  }
  ...
  /**
   * @return the current target if set; null if unset. <b>NOTE</b>: Targets and content nodes are
   *     mutually exclusive, if one is set, the other one is automatically set to null
   */
  ...
  /**
   * @return the current content node if set; null if unset <b>NOTE</b>: Targets and content nodes
   *     are mutually exclusive, if one is set, the other one is automatically set to null
   */
  ...
  public void setGestureEnabled(boolean enable) {
    this.gestureEnabled.set(enable);
  }

  /**
   * @return whether the content node (if set) will be clipped with {@link Node#setClip(Node)} using
   *     a viewport-sized rectangle
   */
  public boolean isClipEnabled() {
    return clipEnabled.get();
  }
  ...
  public void setLockScaleX(boolean lockScaleX) {
    this.lockScaleX.set(lockScaleX);
  }

  public boolean isLockScaleY() {
    return lockScaleY.get();
  }
  ...
  public void setLockScaleY(boolean lockScaleY) {
    this.lockScaleY.set(lockScaleY);
  }
  ...
  public void setBindScale(boolean bindScale) {
    this.bindScale.set(bindScale);
  }
  ...
  /**
   * @return the current x axis offset in <b>target coordinates</b>, see {@link
   *     GesturePane#getTargetViewport()}
   */
  public double getCurrentX() {
    return affine.getTx() / getCurrentScaleX();
  }
  ...
  /**
   * @return the current y axis offset in <b>target coordinates</b>, see {@link
   *     GesturePane#getTargetViewport()}
   */
  public double getCurrentY() {
    return affine.getTy() / getCurrentScaleY();
  }
  ...
  /**
   * @return the scroll zoom factor controlling the constant factor of how much each scroll event
   *     affects the transformation; defaults to 1
   */
  ...
  public void setScrollZoomFactor(double factor) {
    this.scrollZoomFactor.set(factor);
  }

  /** @return the viewport bounding box in <b>target coordinates</b> */
  public Bounds getTargetViewport() {
    return targetRect.get();
  }
  ...
  public enum FitMode {
    ...
    CENTER,
    /**
     * Node will not be scaled nor constrained; this also disables both vertical and horizontal
