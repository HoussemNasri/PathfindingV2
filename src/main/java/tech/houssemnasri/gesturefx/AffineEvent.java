...
import tech.houssemnasri.gesturefx.NamedAffine;
...
public final class AffineEvent extends Event {
  ...
  public static final EventType<AffineEvent> ANY = new EventType<>(Event.ANY, "AFFINE");
  ...
  public Point2D getTargetCentre() {
    return centreOf(targetDimension);
  }

  public Dimension2D getTransformedDimension() {
    return new Dimension2D(
        targetDimension.getWidth() * affine.getMxx(),
  ...
  /**
   * @return a copy of the current affine transformation subtract the previous transformation. This
   *     is the same as direct matrix subtraction: Mat(previous)-Mat(current)
   */
  ...
  static class NamedAffineTransform implements NamedAffine {
    ...
    NamedAffineTransform(Affine affine) {
      this.af = Objects.requireNonNull(affine);
    }
